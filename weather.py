    ...
    for monthf in range(1, 13):
        ...
        for ista in range(len(names)):
            ...
            if len(txt) < 100 or 'error.png' in str(txt):
                ...
                if len(dfmin) == 0 and len(dfmax) == 0 and len(dfrain) == 0:
                    dfmin['date'] = dateu
                    dfmax['date'] = dateu
                    dfrain['date'] = dateu
        ...
        dfmin.to_csv(f'data/tempmin_{year}-{month}.csv', index=False)
        dfmax.to_csv(f'data/tempmax_{year}-{month}.csv', index=False)
        dfrain.to_csv(f'data/rain_{year}-{month}.csv', index=False)
##
for pref in ['tempmax', 'tmpmin', 'rain']:
